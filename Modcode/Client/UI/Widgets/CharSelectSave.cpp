...
#define TITLE_BIGENDER(x, y) \
	{x, y}, {x, y}, {x, y}, { x, y }
#define TITLE_NOGENDER(x) \
	{x, x}, {x, x}, {x, x}, { x, x }
#define TITLE_BIGENDER_EXP(x, y) \
	{x, y}, {x, y}, {x, y}, {x, y}, { x, y }
#define TITLE_NOGENDER_EXP(x) \
	{x, x}, {x, x}, {x, x}, {x, x}, { x, x }
...
namespace D2Widgets
{
	CharSelectSave::CharSelectSave(const char *characterSave, D2SaveHeader &header)
	{
		...
		if (header.nCharTitle != 0)
		{
			...
			if (bIsExpansion)
			{
				if (bIsHardcore)
				{
					characterTitle->SetText(bClassMale ? TitleStatus_ExpansionHardcore[header.nCharTitle].maleTitle : TitleStatus_ExpansionHardcore[header.nCharTitle].femaleTitle);
				}
				else
				{
					characterTitle->SetText(bClassMale ? TitleStatus_Expansion[header.nCharTitle].maleTitle : TitleStatus_Expansion[header.nCharTitle].femaleTitle);
				}
			}
			else if (bIsHardcore)
			{
				characterTitle->SetText(bClassMale ? TitleStatus_ClassicHardcore[header.nCharTitle].maleTitle : TitleStatus_ClassicHardcore[header.nCharTitle].femaleTitle);
			}
			else
			{
				characterTitle->SetText(bClassMale ? TitleStatus_Classic[header.nCharTitle].maleTitle : TitleStatus_Classic[header.nCharTitle].femaleTitle);
			}
		}
	...
	void CharSelectSave::SetDrawPosition(uint32_t x, uint32_t y)
	{
		this->x = x;
		this->y = y;
	...
	bool CharSelectSave::CheckMouseDownInChain(DWORD dwX, DWORD dwY, int &counter)
	{
		if (1)
		{ // mouse down...
			return true;
	...
	bool CharSelectSave::CheckMouseClickInChain(DWORD dwX, DWORD dwY, int &counter)
	{
		if (1)
		{ // mouse clicked...
			return true;
	...
	void CharSelectSave::DrawLink(int counter, bool bDrawLeft)
	{
		// Layout: 2-column x 4-row grid
		// Index 0 from top = (D2_NUM_VISIBLE_SAVES - counter)
		int index = D2_NUM_VISIBLE_SAVES - counter;
		int row = index / 2;
		SetDrawPosition(bDrawLeft ? 35 : 305, 85 + (row * 95));
