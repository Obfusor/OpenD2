...
/*
 * DT1 block info - lightweight struct for modcode access to DT1 block metadata
 */
struct DT1BlockInfo
{
	long orientation;
...
{ // These get imported from the engine
	...
	DWORD (*DT1_GetNumBlocks)(handle dt1);
	bool (*DT1_GetBlockInfo)(handle dt1, int blockNum, DT1BlockInfo *info);
	void *(*DT1_DecodeBlock)(handle dt1, int32_t blockNum, uint32_t &width, uint32_t &height, int32_t &x, int32_t &y);
};
